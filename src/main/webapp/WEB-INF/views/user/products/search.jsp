...
<body>
	...
	<div class="well well-small">
		<div class="row">
			<span style="margin-left: 320px; font-size: 20px;"><b>CÁC SẢN PHẨM BẠN ĐANG TÌM</b><br><br> </span>
			<span style="margin-left: 30px; font-size: 15px;"><b>Từ khóa bạn nhập: </b>${name}<br><br> </span>
	...
	<div class="pagination">
	  <c:forEach var="item" begin="1" end="${paginateInfo.totalPage}" varStatus="loop">
	     <c:if test="${(loop.index) == paginateInfo.currentPage}">
			  <a href="<c:url value="/tim-kiem-san-pham/${name}/${loop.index}" />" class="active">${loop.index}</a>
		 </c:if>
		 
		 <c:if test="${(loop.index) != paginateInfo.currentPage}">
			  <a href="<c:url value="/tim-kiem-san-pham/${name}/${loop.index}" />">${loop.index}</a>
		 </c:if>
